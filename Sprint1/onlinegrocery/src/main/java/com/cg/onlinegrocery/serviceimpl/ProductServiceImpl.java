...
public class ProductServiceImpl extends DBUtils implements ProductService {
	...
	@Override
	public void updateProductQuantity(Product product, int productId, int newQuantity) {
		// TODO Auto-generated method stub
		DBUtils dbutil=new DBUtils();
		EntityManager entityManager=dbutil.createEntityManager();
		entityManager.getTransaction().begin();	
		searchProduct(product.getProductName(), product.getProductId());
	...
	public void updateProductsQtyAfterShopping(Product orderedProduct,int productId, int orderProductQuantity) {
		...
		EntityManager entityManager=dbutil.createEntityManager();
		entityManager.getTransaction().begin();	
		searchProduct(orderedProduct.getProductName(), orderedProduct.getProductId());
		int quantity = orderedProduct.getQuantity();
		quantity= quantity-orderProductQuantity;
